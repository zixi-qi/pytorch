...
class ZeroBubbleAlgorithm(Enum):
    ...
    def from_str(algo):
        if action == "ZB1P":
            return ZeroBubbleAlgorithm.ZB1P
        elif action == "ZB2P":
            return ZeroBubbleAlgorithm.ZB2P
        elif action == "ZBV":
            return ZeroBubbleAlgorithm.ZBV
        else:
            raise RuntimeError(f"Invalid zero bubble algorithm type {algo=}")
...
def _get_1f1b_rank_ops_zero_bubble(
    ...
    forward_local_stage_one_index=None,
):
    # All stages start with handling microbatch 0
    fwd_stage_mb_index: Dict[int, int] = defaultdict(int)
    bwd_stage_mb_index: Dict[int, int] = defaultdict(int)
...
class ScheduleFlexibleInterleaved1F1B(PipelineScheduleMulti):
    ...
    def _calculate_single_rank_operations(self, rank) -> List[Optional[_Action]]:
        ...
        if self.zero_bubble_algorithm:
            ...
            return _get_1f1b_rank_ops_zero_bubble(
                self.n_local_stages,
                self.pp_group_size,
                warmup_ops,
                fwd_bwd_ops,
